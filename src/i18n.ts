...
const dict: Record<string, [string, string]> = {
  ...
  'settings.install.desc': ['没装过 DeepSeek Harness 就点这个：先确认安装目录，再自动下载、安装、配置（含依赖与全局 CLI），几分钟搞定；已有 DSH 但缺依赖/CLI 也会自动补齐', 'Never installed DeepSeek Harness? Click this: confirm the directory, then it downloads, installs and configures everything (dependencies + global CLI); if DSH already exists but tools/CLI are missing, it fills them in automatically'],
  'settings.install.btn': ['安装 DSH 及相关依赖', 'Install DSH & dependencies'],
  ...
  'view.install.btn': ['安装 DSH 及相关依赖', 'Install DSH & dependencies'],
  ...
  'modal.installDesc': ['选择 DeepSeek Harness 的安装目录。将自动完成：①缺失的 git / Node.js / pnpm 一键安装（winget/brew）②克隆 DSH 官方仓库 ③安装依赖并构建（pnpm run build）④全局安装 DSH 命令行工具 dsh（npm i -g @deepseek-ai/dsh@latest）。已有 DSH 但缺依赖/CLI 也会自动补齐。全程无需命令行。', 'Choose where to install DeepSeek Harness. It will: ① install missing git / Node.js / pnpm (winget/brew) ② clone the official DSH repo ③ install dependencies and build (pnpm run build) ④ install the global DSH CLI (npm i -g @deepseek-ai/dsh@latest). If DSH already exists but tools/CLI are missing, they are filled in automatically. No command line needed.'],
