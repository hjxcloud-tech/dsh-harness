...
export const PLUGIN_CHANGELOG: ChangelogEntry[] = [
  ...
  },
  {
    ...
    items: [
      ...
      [
        '**只认官方 DSH 包**：本机版本只从官方包清单读取，第三方同名社区包（`@x1a0f3n9/dsh-*`，版本号自成一套）不再被当成本体；「升级前结束全部 DSH」的进程范围也按官方身份收窄',
        '**Only the official DSH package counts**: the local version comes from the official package manifest, so third-party same-named packages (`@x1a0f3n9/dsh-*`, which version independently) are no longer mistaken for it; the pre-upgrade "stop all DSH" scope is narrowed to official identities too',
      ],
      [
