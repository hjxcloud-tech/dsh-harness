...
| One-click configure DSH | button | Auto-install dependencies → clone (live percentage) → install dependencies (progress bar) → auto-configure |
...
├── icon.ts                  # Official DeepSeek whale logo (SVG, theme-aware)
...
├── bridge-mode.ts           # Bridge mode types + legacy boolean migration (pure)
├── source-tag.ts            # Bridge implicit info line: packs selection pos/count/path into chat
...
├── detector.ts              # DSH repo/CLI detection (candidate dirs + PATH probe)
├── installer.ts             # One-click install: deps (winget/npm/mirror) + clone + build + CLI
├── install-progress-modal.ts # One-click configure install progress modal (steps + progress bar)
...
├── settings.ts              # Settings tab (language / bridge / quick actions / advanced / diagnostics)
├── changelog.ts             # In-app plugin changelog modal
├── changelog-data.ts        # Plugin changelog data (pure data)
...
1. 启用插件，侧边栏出现鲸鱼图标
2. 第一次用：设置 →「一键配置 DSH」（或已装过 DSH 则点「一键检测配置」）
3. 点侧边栏图标打开面板，即开即用
...
| 一键配置 DSH | 按钮 | 自动装依赖 → 克隆（实时百分比）→ 装依赖（进度条）→ 自动配置 |
...
| Obsidian 桥接到 DSH 聊天框 | 开 | 开启后，框选文字右键即可发送到 DSH 聊天框（命令面板同样可用）；发送后自动打开 DSH 面板查看处理 |
| DSH 聊天框桥接到 Obsidian | 自动发送 | 控制 DSH 与 Obsidian 之间的桥接：自动发送（框选文字自动以隐式信息行注入聊天框，含精确行:列与字数，不含原文）、右键发送（仅通过右键菜单/命令发送）、取消（关闭桥接）；非「取消」时 DSH 产物中的库内可读路径点击即可在 Obsidian 打开 |
...
| `EADDRINUSE` / 端口 3080 被占用 | 残留 DSH 进程占着端口 | 设置 →「快捷功能」→「重启服务」 |
| 「桥接未就绪，改为直接发送」 | 桥接脚本尚未注入面板 | 设置 →「快捷功能」→「重启服务」（插件更新后请先彻底重启 Obsidian 再重启 DSH 服务） |
